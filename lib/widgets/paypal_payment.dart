...
class _PaypalPaymentButtonState extends State<PaypalPaymentButton> with CouponCodeMixin {
  ...
  getIdKeyCurrency() async {
    ...
    try{
      await _firestore.collection('Notice').doc('paypal_money').get().then((value) {
  ...
  paymentCheckOut(transactions, accessToken) async {
    var headers = {
      'Authorization': 'Basic QWVUd1FoN2lRNXM4OU9MclVaNEJ3bllDMEM1b1NrYWtuQWt5RG1VLW50cmFycExUSjRFMzVhdml5S2NibHQzeDkwLUJzZE04azl4c3hNaUo6RUpKVnFZMHlheWxCTTk4U2FxcGRTRG9mbTNxSW5CcHg5X1FsTHFoTnJFam5Tc0ZtOUVTN2hCYmE3MDJLUm9xREZWZHRFYlhKdHFrV1IyVm8=',
            ...
            onTap: () async {
              ...
              Future.delayed(Duration.zero, () async {
                try{
                  print('Accessss token');
                  accessToken1 = await payPalServices.getAccessToken();
                  final response  = await paymentCheckOut(transactions, accessToken1);
                  print('response = ${response}');
                  checkoutUrl = response["approvalUrl"];
                  executeUrl = response["executeUrl"];
                  setState(() {});
